...
void main() {
  ...
  test('ГЛАВНЫЙ ТЕСТ (круг правки 1): авария между выдачей номера и записью '
      'настоящей строки не оставляет в базе сироту без состояния', () async {
    final numbers = ReceiptNumbers(db);
  ...
  test('строка, вставленная через withNext, рождается сразу полной — '
      'без промежуточного состояния без состояния', () async {
    final numbers = ReceiptNumbers(db);
  ...
  test('круг правки 2: конфликт по чужому уникальному ключу внутри writeSale '
      'не запускает повтор', () async {
  ...
  test('круг правки 3: исчерпание попыток бросает ReceiptNumberExhausted с '
      'кассой и числом попыток, а не тихо зависает', () async {
    ...
    await expectLater(
      ...
      throwsA(
        isA<ReceiptNumberExhausted>()
            ...
            .having((e) => e.attempts, 'attempts', ReceiptNumbers.maxAttempts),
      ),
    );
  ...
  test('ReceiptNumberExhausted называет кассу и число попыток в тексте — '
