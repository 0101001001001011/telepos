...
void main() {
  ...
  test('в пул не попадает чужая касса и чек не в том состоянии', () async {
    // Правило нуля (`qa-depth`): в базе лежат записи, которые **не должны**
    // попасть в ответ, иначе «список пуст» и «фильтр работает» неотличимы.
    var view = await cart.start(terminalId: 7, wholesale: false, meta: m(1, 0));
    view = await cart.addByBarcode(7, barcodeA, mv(view, 2));
    ...
    final pool = await cart.watchDeferred(by: fullDiscountAuthority).first;
    expect(pool.map((c) => c.receiptNo), [mine]);
  });
