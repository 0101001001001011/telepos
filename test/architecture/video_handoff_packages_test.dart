...
void main() {
  ...
  test('обязательные поля заполнены и версия — настоящая', () {
    ...
    for (final pkg in packages()) {
      ...
      // ── Версия: настоящая, но НЕ обязательно сегодняшняя ──────────
      //
      // Стояло `equals(version)` — равенство текущему `pubspec.yaml`. На
      // выпуске 3.7.0 это разом покрасило четыре сданных пакета: дубли
      // сняты сборкой 3.6.0+21, и поднятие версии задним числом не делает
      // эту запись ложной. Наоборот: переписать её значило бы соврать о
      // том, чем снимали.
      //
      // Проверяется то, ради чего сторож и заведён: версия ВЫГЛЯДИТ как
      // версия и не из будущего. Выдуманное «1.0.0» или «4.0.0+99» он
      // по-прежнему ловит, а честную историю не трогает.
      final shot = m['version'] as String?;
      expect(
        shot,
        matches(RegExp(r'^\d+\.\d+\.\d+\+\d+$')),
        reason:
            '$where: версия обязана быть НАСТОЯЩЕЙ версией сборки, которой '
            'снят дубль, в виде X.Y.Z+N. В манифесте "$shot"',
      );
      expect(
        _order(shot!) <= _order(version),
        isTrue,
        reason:
            '$where: версия дубля из будущего — снять дубль сборкой, '
            'которой ещё нет, нельзя. В манифесте "$shot", '
            'в pubspec "$version"',
      );
    }
  });
...
/// Версия числом, чтобы сравнивать по порядку, а не по написанию:
/// «3.10.0» строкой меньше «3.9.0», и сторож пропустил бы будущее.
int _order(String v) {
  final m = RegExp(r'^(\d+)\.(\d+)\.(\d+)\+(\d+)$').firstMatch(v);
  if (m == null) return -1;
  final p = [1, 2, 3, 4].map((i) => int.parse(m.group(i)!)).toList();
  return ((p[0] * 1000 + p[1]) * 1000 + p[2]) * 100000 + p[3];
}
