...
void main() {
  ...
  testWidgets('НДС: ставка страны показана в строке плательщика', (
    ...
    // Ставку проба больше НЕ ищет числом: она приходит из набора страны,
    // читается из ресурсов и в виджет-пробе не успевает. Числа в коде
    // нет по построению — 2026-09-22 поле `vatRate` снято с
    // перечисления: закон меняет ставку, а сборка выходит не в тот же
    // день.
    //
    // Проверяется то, ради чего шаг и нужен: вопрос про НДС задан.
    expect(find.byType(SettingsChoiceTile), findsWidgets);
  });
