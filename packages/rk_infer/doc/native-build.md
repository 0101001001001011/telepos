...
`rk_infer` is a **Flutter FFI plugin**. Cargo is invoked from the per-platform
...
| --- | --- |
| Windows, Linux | CMake: `src/CMakeLists.txt`, the path handed over in `rk_infer_bundled_libraries` |
| Android | Gradle directly: the `rkInferCargoBuild` task in `android/build.gradle`, the result placed in `jniLibs.srcDirs` |
...
`x86` is built on request: `-PrkInferAbiFilter=x86`.
...
| `windows/CMakeLists.txt` | pulls in `src/`, hands the path over in `rk_infer_bundled_libraries` |
| `linux/CMakeLists.txt` | the same |
| `android/build.gradle` | the `rkInferCargoBuild` task, the mapping of ABI onto triple and onto the NDK clang wrapper, `jniLibs.srcDirs` |
...
| `ios/rk_infer.podspec`, `macos/rk_infer.podspec` | CocoaPods: the script phase and `-force_load` |
...
`rk_infer.framework/rk_infer` — a single Mach-O. That means Apple gets a
...
```sh
cd packages/rk_infer/rust
cargo build --release
cargo test
cargo clippy --all-targets -- -D warnings
```

The binding opens the library at `bindingLibraryPath` — the path is required and
is never guessed. Inside a built application that is the platform's own name,
which the system loader finds by itself: `librk_infer.so` on Linux and Android,
`rk_infer.dll` on Windows.

## What arrived, and what is only written

| Target | State | Evidence |
| --- | --- | --- |
| Windows | arrived | `rk_infer.dll` next to the runner of a built application |
| Linux | arrived | `librk_infer.so` in the application's `bundle/lib/` |
...
| macOS | arrived | a C probe links against `librk_infer.a` with `-force_load` and runs, arm64 and x86_64, Release and Debug (2026-08-03) |
