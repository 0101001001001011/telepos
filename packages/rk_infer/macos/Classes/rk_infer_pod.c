...
// translation unit is the minimum that makes `rk_infer.framework/rk_infer`
// exist; the symbols that matter are pulled in from the Rust static archive by
// `-force_load` (see ../rk_infer.podspec and ../../apple/build_rust.sh).
//
// Referenced by nothing on purpose: it is scaffolding, not an entry point.

__attribute__((used)) static const char rk_infer_pod_marker[] = "rk_infer";
