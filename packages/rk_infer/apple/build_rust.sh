...
# building, and leaves one fat archive at $BUILT_PRODUCTS_DIR/librk_infer.a.
#
...
# becomes `rk_infer.framework`, and Dart opens it as
# `rk_infer.framework/rk_infer` — a single Mach-O binary. So the cargo artefact
...
if ! command -v cargo >/dev/null 2>&1; then
  echo "rk_infer: cargo is on neither PATH nor ${CARGO_HOME:-$HOME/.cargo}/bin." >&2
  echo "rk_infer: install Rust (https://rustup.rs) or set CARGO_HOME." >&2
  exit 1
fi
...
triple_for() {
  ...
  case "$PLATFORM_NAME:$arch" in
    ...
    *)
      echo "rk_infer: no Rust triple for PLATFORM_NAME=$PLATFORM_NAME arch=$arch" >&2
      exit 1
      ;;
  esac
}
...
for arch in $ARCHS; do
  ...
  SLICES="$SLICES $TARGET_DIR/$triple/$PROFILE_DIR/librk_infer.a"
done
...
lipo -create $SLICES -output "$BUILT_PRODUCTS_DIR/librk_infer.a"
...
if ! nm -gU "$BUILT_PRODUCTS_DIR/librk_infer.a" 2>/dev/null | grep -q " _rk_infer_abi_version$"; then
  echo "rk_infer: librk_infer.a carries no rk_infer_abi_version -- the C ABI did not survive the build" >&2
  exit 1
fi
