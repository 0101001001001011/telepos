...
# building, and leaves one fat archive at $BUILT_PRODUCTS_DIR/librk_syslog.a.
#
...
# becomes `rk_syslog.framework`, and Dart opens it as
# `rk_syslog.framework/rk_syslog` — a single Mach-O binary. So the cargo artefact
...
if ! command -v cargo >/dev/null 2>&1; then
  echo "rk_syslog: cargo is on neither PATH nor ${CARGO_HOME:-$HOME/.cargo}/bin." >&2
  echo "rk_syslog: install Rust (https://rustup.rs) or set CARGO_HOME." >&2
  exit 1
fi
...
# already declare.
#
# Measured 2026-08-03: this package alone builds for aarch64-apple-ios WITHOUT
# the variable, because its crypto is `ring` and not `aws-lc-sys` -- the
# ___chkstk_darwin calls come from aws-lc's large-frame C, which rk_pki and
# rk_quic do link and this one does not. The export stays: it costs nothing and
# a dependency bump must not silently reintroduce the failure.
...
triple_for() {
  ...
  case "$PLATFORM_NAME:$arch" in
    ...
    *)
      echo "rk_syslog: no Rust triple for PLATFORM_NAME=$PLATFORM_NAME arch=$arch" >&2
      exit 1
      ;;
  esac
}
...
for arch in $ARCHS; do
  ...
  SLICES="$SLICES $TARGET_DIR/$triple/$PROFILE_DIR/librk_syslog.a"
done
...
lipo -create $SLICES -output "$BUILT_PRODUCTS_DIR/librk_syslog.a"
...
if ! nm -gU "$BUILT_PRODUCTS_DIR/librk_syslog.a" 2>/dev/null | grep -q " _rk_syslog_version$"; then
  echo "rk_syslog: librk_syslog.a carries no rk_syslog_version -- the C ABI did not survive the build" >&2
  exit 1
fi
