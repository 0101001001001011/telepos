...
A Rust toolchain is therefore required to check a change here. `RK_SYSLOG_LIB`
points the Dart suite at a library somewhere other than
`rust/target/{release,debug}`; without it, the suite finds the one the build
above just produced.
