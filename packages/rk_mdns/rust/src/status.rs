...
//! `static` storage — nothing is allocated, so nothing has to be freed (И146).

use std::ffi::c_char;

/// Every way a call into this library can end.
///
/// Small and closed on purpose: a caller must be able to write a total
/// `switch` over it. Detail that varies per occurrence — which port, which
/// interface — belongs in the message from `rk_mdns_last_error`, not in a new
/// variant.
...
pub enum Status {
    ...
    ///
    /// Its own variant rather than a kind of `BindFailed`, because it is the
    /// expected condition rather than a fault: Chrome holds 5353 on Windows
    /// whenever it is running, `avahi-daemon` holds it on Linux, and
    /// `mDNSResponder` holds it on every Mac. A caller that sees this knows to
    /// look for another responder, not for a bug.
    PortInUse,
    /// The socket could not be bound for another reason — no permission, no
    /// such address.
    BindFailed,
    /// This host has no interface to announce on, or none that accepted a
    /// socket.
    ///
    /// Deliberately not survivable by falling back to a default send. A
    /// datagram sent without `IP_MULTICAST_IF` leaves by whichever interface
    /// the routing table prefers, which was measured to be a virtual switch
    /// nothing is behind.
    NoInterface,
    ...
    NotRunning,
    /// A panic was caught at the boundary. The process is still alive; the
    /// call did nothing. Seeing this is a bug in this library, and the caller
    /// is entitled to be told rather than to be aborted.
    Panic,
...
impl Status {
    ...
    ///
    /// `lowerCamelCase` so it matches a Dart enum name with no translation
    ...
    pub const fn name_with_nul(self) -> &'static str {
        match self {
            ...
            Status::NoInterface => "noInterface\0",
