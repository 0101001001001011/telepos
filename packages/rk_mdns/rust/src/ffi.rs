//! The C ABI of `rk_mdns`, and the three rules it keeps.
//!
...
//!    allocated by Rust that the caller returns with [`rk_mdns_string_free`].
//!    Dart never calls `malloc`/`free` on memory that crossed this boundary.
...
pub const RK_MDNS_ABI_VERSION: u32 = 1;
...
#[no_mangle]
pub extern "C" fn rk_mdns_abi_version() -> u32 {
    RK_MDNS_ABI_VERSION
}
...
#[no_mangle]
pub extern "C" fn rk_mdns_version() -> *const c_char {
...
/// `rk_mdns_string_free`". Passing null is a no-op; passing anything else — a
/// `static` pointer, a Dart pointer, a pointer already freed — is undefined
...
/// freed yet.
#[no_mangle]
pub unsafe extern "C" fn rk_mdns_string_free(s: *mut c_char) {
    if s.is_null() {
        return;
    }
