...
# building, and leaves one fat archive at $BUILT_PRODUCTS_DIR/librk_mdns.a.
#
...
# becomes `rk_mdns.framework`, and Dart opens it as
# `rk_mdns.framework/rk_mdns` — a single Mach-O binary. So the cargo artefact
...
# runtime, by name.
#
# Everything below was learned the hard way on 2026-08-03, the first day any
# Apple line in these packages had ever been compiled. Four defects that day,
# two of which made a published package useless to every consumer.
...
# the pod build dies with `cargo: command not found`.
if ! command -v cargo >/dev/null 2>&1; then
  export PATH="${CARGO_HOME:-$HOME/.cargo}/bin:$PATH"
fi
if ! command -v cargo >/dev/null 2>&1; then
  echo "rk_mdns: cargo is on neither PATH nor ${CARGO_HOME:-$HOME/.cargo}/bin." >&2
  echo "rk_mdns: install Rust (https://rustup.rs) or set CARGO_HOME." >&2
  exit 1
fi
...
# The Rust link step and any C compiled through cc-rs must agree on the
...
# recent iPhoneOS SDKs hide through iOS 12.1, and the link dies with
#
...
# version mismatch. Of the five Apple triples only aarch64-apple-ios fails that
# way, because the simulator triples default to 14.0 and aarch64-apple-darwin
# to 11.0.
#
...
triple_for() {
  ...
  case "$PLATFORM_NAME:$arch" in
    ...
    *)
      echo "rk_mdns: no Rust triple for PLATFORM_NAME=$PLATFORM_NAME arch=$arch" >&2
      exit 1
      ;;
  esac
}
...
for arch in $ARCHS; do
  ...
  # 2026-08-03 on a sibling package, 0 of its 7 entry points present in the .a
  # while the .dylib from the same invocation carried all 7. The trigger is
  # `rlib` being emitted alongside -- staticlib+cdylib is fine, staticlib+rlib
  # is not -- and rlib cannot leave Cargo.toml because tests/ link against it.
  # Restricting this one invocation to a single crate type restores the
  # symbols and keeps LTO.
  ( cd "$CRATE_DIR" && cargo rustc $CARGO_PROFILE_ARG --target "$triple" \
      --target-dir "$TARGET_DIR" --crate-type staticlib )
  SLICES="$SLICES $TARGET_DIR/$triple/$PROFILE_DIR/librk_mdns.a"
done
...
lipo -create $SLICES -output "$BUILT_PRODUCTS_DIR/librk_mdns.a"
...
# archive cleanly: nm still rejects a few dozen objects with "Unknown attribute
# kind (102)" -- every one of them compiler_builtins out of the Rust sysroot --
# which is why stderr goes to /dev/null. What the single crate type buys is
# that the ONE object carrying the #[no_mangle] entry points is a readable
# Mach-O; with `rlib` also emitted that object is __LLVM,__bitcode too and the
# grep below finds nothing.
if ! nm -gU "$BUILT_PRODUCTS_DIR/librk_mdns.a" 2>/dev/null | grep -q " _rk_mdns_abi_version$"; then
  echo "rk_mdns: librk_mdns.a carries no rk_mdns_abi_version -- the C ABI did not survive the build" >&2
  exit 1
fi
