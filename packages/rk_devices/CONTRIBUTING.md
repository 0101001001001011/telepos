...
A Rust toolchain is therefore required to check a change here, and the crate
deliberately has no dependencies: it must build on a machine that has never
seen the network, because it builds for tills.
...
a different thing. So is a change to what a byte sequence parses into — a
caller is relying on the reading, not on the signature that returns it.
