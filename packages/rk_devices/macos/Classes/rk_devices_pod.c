...
// translation unit is the minimum that makes `rk_devices.framework/rk_devices`
// exist; the symbols that matter are pulled in from the Rust static archive by
// `-force_load` (see ../rk_devices.podspec and ../../apple/build_rust.sh).
//
// Referenced by nothing on purpose: it is scaffolding, not an entry point.

__attribute__((used)) static const char rk_devices_pod_marker[] = "rk_devices";
