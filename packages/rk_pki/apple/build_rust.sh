...
# building, and leaves one fat archive at $BUILT_PRODUCTS_DIR/librk_pki.a.
#
...
# becomes `rk_pki.framework`, and Dart opens it as
# `rk_pki.framework/rk_pki` — a single Mach-O binary. So the cargo artefact
...
if ! command -v cargo >/dev/null 2>&1; then
  echo "rk_pki: cargo is on neither PATH nor ${CARGO_HOME:-$HOME/.cargo}/bin." >&2
  echo "rk_pki: install Rust (https://rustup.rs) or set CARGO_HOME." >&2
  exit 1
fi
...
triple_for() {
  ...
  case "$PLATFORM_NAME:$arch" in
    ...
    *)
      echo "rk_pki: no Rust triple for PLATFORM_NAME=$PLATFORM_NAME arch=$arch" >&2
      exit 1
      ;;
  esac
}
...
for arch in $ARCHS; do
  ...
  SLICES="$SLICES $TARGET_DIR/$triple/$PROFILE_DIR/librk_pki.a"
done
...
lipo -create $SLICES -output "$BUILT_PRODUCTS_DIR/librk_pki.a"
...
if ! nm -gU "$BUILT_PRODUCTS_DIR/librk_pki.a" 2>/dev/null | grep -q " _rk_pki_abi_version$"; then
  echo "rk_pki: librk_pki.a carries no rk_pki_abi_version -- the C ABI did not survive the build" >&2
  exit 1
fi
