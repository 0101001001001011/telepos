...
`rk_pki` is a **Flutter FFI plugin**. Cargo is invoked from the per-platform
build files; the artefact is handed to the Flutter tool, which puts it in the
...
| --- | --- |
| Windows, Linux | CMake: `src/CMakeLists.txt`, the path handed over in `rk_pki_bundled_libraries` |
| Android | Gradle directly: the `rkPkiCargoBuild` task in `android/build.gradle`, the result placed in `jniLibs.srcDirs` |
...
x86. `x86` is built on request: `-PrkPkiAbiFilter=x86`.
...
| `windows/CMakeLists.txt` | pulls in `src/`, hands the path to `rk_pki_bundled_libraries` |
| `linux/CMakeLists.txt` | the same |
| `android/build.gradle` | the `rkPkiCargoBuild` task, the ABI-to-triple and ABI-to-NDK-clang-wrapper maps, `jniLibs.srcDirs` |
...
| `ios/rk_pki.podspec`, `macos/rk_pki.podspec` | CocoaPods: the script phase and `-force_load` |
...
`rk_pki.framework/rk_pki` — a single Mach-O. That means Apple gets the
...
loader, so inside a built application it finds itself;
`RkPkiLibrary.open(path: ...)` points it at a particular file instead — for
example at the artefact from a built application.

## What arrived, and what is only written

| Target | State | Evidence |
| --- | --- | --- |
| Windows | arrived | `rk_pki.dll` next to the runner of a built application |
| Linux | arrived | `librk_pki.so` in the application's `bundle/lib/` |
...
| macOS | arrived | a C probe links against `librk_pki.a` with `-force_load` and runs, arm64 and x86_64, Release and Debug (2026-08-03) |
