...
// translation unit is the minimum that makes `rk_zenoh.framework/rk_zenoh`
// exist; the symbols that matter are pulled in from the Rust static archive by
// `-force_load` (see ../rk_zenoh.podspec and ../../apple/build_rust.sh).
//
// Referenced by nothing on purpose: it is scaffolding, not an entry point.

__attribute__((used)) static const char rk_zenoh_pod_marker[] = "rk_zenoh";
