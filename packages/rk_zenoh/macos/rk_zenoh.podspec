...
pod framework so `dart:ffi` can open it as rk_zenoh.framework/rk_zenoh.
  ...
  s.pod_target_xcconfig = {
    ...
    #
    # Цепочка: zenoh -> zenoh-link-commons -> quinn -> quinn-proto ->
    # rustls-platform-verifier 0.7.0 -> security-framework 3.7.0.
    # Измерено 2026-08-03: без Security -- 292 неразрешённых символа
    # (_CMSDecoderCopyAllCerts, _AuthorizationCreate,
    # _SecTrustEvaluateWithError, _SecPolicyCreateSSL); без CoreFoundation
    # -- 90 (_CFArrayCreate, _CFBundleGetMainBundle, _CFDataCreate).
    #
    # В отличие от rk_nats и rk_quic, у этого пакета та же строка стоит и в
    # ios/: rustls-platform-verifier тянет security-framework на всех трёх
    # Apple-таргетах, а не только там, где есть keychain.
