...
`rk_zenoh` is a **Flutter FFI plugin**. Cargo is invoked from the per-platform
...
| --- | --- |
| Windows, Linux | CMake: `src/CMakeLists.txt`, the path is handed to `rk_zenoh_bundled_libraries` |
| Android | Gradle directly: the `rkZenohCargoBuild` task in `android/build.gradle`, output placed in `jniLibs.srcDirs` |
...
is built on request: `-PrkZenohAbiFilter=x86`.
...
| `windows/CMakeLists.txt` | includes `src/`, hands the path to `rk_zenoh_bundled_libraries` |
| `linux/CMakeLists.txt` | the same |
| `android/build.gradle` | the `rkZenohCargoBuild` task, ABI → triple and ABI → NDK clang wrapper mappings, `jniLibs.srcDirs` |
...
| `ios/rk_zenoh.podspec`, `macos/rk_zenoh.podspec` | CocoaPods: the script phase and `-force_load` |
...
`rk_zenoh.framework/rk_zenoh` — a single Mach-O. That means Apple gets a
...
```sh
cd packages/rk_zenoh/rust
cargo build --release
cargo test
cargo clippy --all-targets -- -D warnings
```

The binding looks the library up by its platform name through the system loader,
so inside a built application it finds it by itself; `RK_ZENOH_LIBRARY` points it
somewhere else — at the artefact from a built application, for instance.

## What arrived, and what is merely written

| Target | State | Evidence |
| --- | --- | --- |
| Windows | arrived | `rk_zenoh.dll` next to the runner of the built application |
| Linux | arrived | `librk_zenoh.so` in the built application's `bundle/lib/` |
...
| macOS | arrived | a C probe links against `librk_zenoh.a` with `-force_load` and runs, arm64 and x86_64, Release and Debug (2026-08-03) |
