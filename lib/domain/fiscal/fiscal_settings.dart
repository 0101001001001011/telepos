...
class FiscalSettings {
  ...
  /// Вписаны ли в кассу боевые реквизиты оператора.
  ///
  /// Любого из двух достаточно: регистрационный номер выдаёт налоговый орган,
  /// заводской — оператор, и заполненный хоть один означает, что кассу с
  /// оператором уже знакомили. Требовать оба значило бы считать небоевой
  /// кассу, у которой заполнена ровно половина, — а такая касса чеки шлёт.
  bool get hasLiveOperatorCredentials =>
      (registrationNumber?.trim().isNotEmpty ?? false) ||
      (cashboxUniqueNumber?.trim().isNotEmpty ?? false);

  /// Можно ли направлять эту кассу на **эмулятор** фискального оператора.
  ///
  /// # Почему это свойство данных, а не разрешение человека
  ///
  /// Решение заказчика 2026-09-19: встроенный эмулятор ОФД включается только
  /// пока касса не боевая, и «не боевая» определяется тем, что в кассе
  /// записано, а не тем, что оператор про неё думает. Намерение здесь не
  /// годится: «я только проверю шаблон чека» — обычная и честная мысль, после
  /// которой касса остаётся смотреть на эмулятор, а покупатель уносит бумажку
  /// без документа.
  ///
  /// Два входа, и оба нужны:
  ///
  /// * `testMode` — касса объявлена испытательной **самой настройкой**; на
  ///   боевой её не ставят, потому что в этом режиме и адрес оператора
  ///   испытательный (`FiscalDefaults.cloudBaseUrl`);
  /// * пустые реквизиты — кассу с оператором ещё не знакомили, фискализовать
  ///   ей нечем в принципе.
  ///
  /// # Чего это свойство НЕ доказывает
  ///
  /// Что на кассе не пробивали настоящих чеков. Реквизиты можно стереть, а
  /// документы за прошлую смену от этого никуда не денутся — свойство
  /// отвечает про **сейчас**, и запрет тем и ценен, что закрывается обратно в
  /// ту же секунду, как реквизиты вписали.
  ///
  /// Не доказывает оно и того, что касса не смотрит на эмулятор: адрес можно
  /// вписать руками, минуя выключатель. Это ловит другая проверка — пометка
  /// по адресу оператора на вкладке диагностики.
  bool get allowsEmulatedOperator => testMode || !hasLiveOperatorCredentials;
