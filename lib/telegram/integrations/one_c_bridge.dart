import 'dart:async';

import 'package:telepos/telegram/core/tdlib_logger.dart';
import 'package:telepos/telegram/data_exchange/telegram_sync_engine.dart';
import 'package:telepos/telegram/integrations/integration_protocol.dart';
import 'package:telepos/domain/entities/telegram/sync_packet.dart';

class OneCBridge implements IntegrationBridge {
  final TelegramSyncEngine _syncEngine;
  final TdLibLogger _logger;

  final String _posId;
  final String _oneCId;
  final String _sessionId;

  bool _connected = false;
  final _incomingController = StreamController<BridgeMessage>.broadcast();

  OneCBridge({
    required TelegramSyncEngine syncEngine,
    required TdLibLogger logger,
    required String posId,
    required String oneCId,
    required String sessionId,
  }) : _syncEngine = syncEngine,
       _logger = logger,
       _posId = posId,
       _oneCId = oneCId,
       _sessionId = sessionId;

  @override
  String get bridgeId => '1c_${_posId}_$_oneCId';

  @override
  BridgeType get bridgeType => BridgeType.oneC;

  @override
  bool get isConnected => _connected;

  @override
  Stream<BridgeMessage> get incomingData => _incomingController.stream;

  @override
  Future<void> connect() async {
    _logger.logConnection('1C bridge connecting...');
    _connected = true;
    _logger.logConnection('1C bridge connected');
  }

  @override
  Future<void> disconnect() async {
    _connected = false;
    await _incomingController.close();
    _logger.logConnection('1C bridge disconnected');
  }

  @override
  Future<void> sendData(ExchangeType type, Map<String, dynamic> data) async {
    if (!_connected) throw StateError('1C bridge not connected');

    await _syncEngine.sendPacket(
      sourceId: _posId,
      targetId: _oneCId,
      type: type,
      data: {'target': '1C', ...data},
      sessionId: _sessionId,
    );
  }

  @override
  Future<Map<String, dynamic>> requestData(
    ExchangeType type, {
    Map<String, dynamic>? params,
  }) async {
    if (!_connected) throw StateError('1C bridge not connected');

    await sendData(type, {'action': 'request', 'params': params});

    final response = await incomingData
        .where((msg) => msg.type == type)
        .first
        .timeout(const Duration(seconds: 60));

    return response.data;
  }

  @override
  Future<bool> ping() async {
    try {
      await sendData(ExchangeType.heartbeat, {
        'ping': true,
        'timestamp': DateTime.now().toIso8601String(),
      });
      return true;
    } catch (_) {
      return false;
    }
  }

  Future<void> requestNomenclature({DateTime? since}) async {
    await sendData(ExchangeType.product, {
      'action': '1c_nomenclature_request',
      'since': since?.toIso8601String(),
    });
  }

  Future<void> uploadSales(List<Map<String, dynamic>> sales) async {
    await sendData(ExchangeType.sale, {
      'action': '1c_sales_upload',
      'items': sales,
      'count': sales.length,
    });
  }

  Future<void> requestStockBalances() async {
    await sendData(ExchangeType.product, {'action': '1c_stock_request'});
  }
}
