...
typedef WireWatchHandler =
    Stream<Map<String, Object?>> Function(Map<String, Object?> body);
...
class TillWire {
  ...
  Future<void> _onRequest(int sessionId, int streamId, String message) async {
    ...
    try {
      verdict = await _guard.check(frame.op, frame.body, frame.token);
    ...
    if (ask != null) {
      ...
      // раньше, а всё, что сторож решает по сеансу (включая владение
      // терминалом), решено до этой строки — обработчику передавать нечего
      // сверх `sessionId`, см. докстринг [WireHandler].
      await _answer(sessionId, streamId, ask, frame.body);
      return;
    }

    final watch = _watchHandlers[frame.op];
    if (watch != null) {
      _subscribe(sessionId, streamId, watch, frame.body);
      return;
    }
  ...
  Future<void> _answer(
    ...
    try {
      ...
      reply = OkFrame(await handler(body, _sessionKey(sessionId)));
