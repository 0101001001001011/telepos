class CouchDbDocumentMapper {
  ...
  static Map<String, dynamic> saleToDoc({
    ...
    return {
      ...
      // Кто занял отложенный чек. `null` — свободен, его может поднять любая
      // касса. Разбор правила целиком — в [claimedByPosId].
      'claimed_by_pos_id': sale['claimed_by_pos_id'],
      'products': products,
      'payments': payments,
    };
  }

  /// Какая касса заняла этот отложенный чек, либо `null` — свободен.
  ///
  /// # Зачем поле, которого не было
  ///
  /// Решение заказчика 2026-09-19: отложенный чек обязан подниматься с
  /// **соседней** кассы. Покупатель отложил корзину на первой, пошёл
  /// дозабрать товар, вернулся — а там очередь, и он идёт ко второй.
  /// Деньги получает та касса, которая пробьёт, — продажи ещё нет, и
  /// спорить не о чем.
  ///
  /// Но как только чек виден двум кассам, обе могут его поднять, и одна
  /// корзина продастся дважды. Опрос раз в пять минут этого не
  /// предотвращает: вторая касса узнает о подъёме через минуты.
  ///
  /// # Занятость решается ревизией, а не полем
  ///
  /// Само поле ничего не гарантирует — гарантирует **способ** его записи:
  /// подъём это «сравни и запиши» по текущей ревизии документа. Кто успел,
  /// тот получил `ok`; опоздавший получает `409 conflict` и узнаёт, что
  /// чек занят, — не тишину и не вторую продажу. Проверка происходит на
  /// сервере, атомарно, и ровно поэтому шаг 2 спеки (отправка ревизий) был
  /// обязателен раньше этой работы.
  ///
  /// # Без связи чужой чек не поднимается
  ///
  /// Исключительность негарантируема, когда сервера нет. Свой отложенный
  /// чек по-прежнему поднимается без всякой сети: он не виден никому
  /// другому. Чужой — отказом с причиной. Попросить покупателя вернуться к
  /// своей кассе дешевле, чем продать корзину дважды.
  static int? claimedByPosId(Map<String, dynamic> doc) =>
      doc['claimed_by_pos_id'] as int?;

  /// Строки чека из документа — обратная половина `_saleLineToMap`.
  ///
  /// До этой работы её не было вовсе: чек уезжал наверх шапкой, и разбирать
  /// строки было незачем. Отложенному чеку без строк грош цена — соседняя
  /// касса подняла бы пустую корзину.
  ///
  /// Деньги разбираются `Decimal.parse` из строки, а не из числа: `double`
  /// потерял бы третий знак (И159), а вес и цена хранятся с тремя.
  /// Нечитаемая строка **пропускается с именем товара в журнале**, а не
  /// роняет весь чек: одна испорченная позиция не повод потерять корзину,
  /// но и молча её недосчитать нельзя.
  static List<Map<String, dynamic>> docToSaleLines(Map<String, dynamic> doc) {
    final raw = doc['products'];
        ...
        if (line is Map)
          {
            ...
            'price_before': line['price_before']?.toString(),
          },
    ];
  }
